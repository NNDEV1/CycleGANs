...
class CycleGAN(object):
    def __init__(self, width, height, xchan, ychan, lambda_=10., pool_size=50, lr=0.0002, beta1=0.5):
        ...
        self._gx_from_gy = Generator('Gx', self._gy['l15']['fmap'], xchan, reuse=tf.AUTO_REUSE)
        self._gy_from_gx = Generator('Gy', self._gx['l15']['fmap'], ychan, reuse=tf.AUTO_REUSE)

        self._real_dx = Discriminator('Dx', self._d_xs)
        self._fake_dx = Discriminator('Dx', self._xs, reuse=tf.AUTO_REUSE)
        self._fake_dx_g = Discriminator('Dx', self._gx['l15']['fmap'], reuse=tf.AUTO_REUSE)
        self._real_dy = Discriminator('Dy', self._d_ys)
        self._fake_dy = Discriminator('Dy', self._ys, reuse=tf.AUTO_REUSE)
        self._fake_dy_g = Discriminator('Dy', self._gy['l15']['fmap'], reuse=tf.AUTO_REUSE)
